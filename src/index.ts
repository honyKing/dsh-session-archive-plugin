...
function resolveScriptsDir(scriptsDir: string | undefined): string {
...
function runPython(script: string, args: readonly string[], config: Config): Promise<{
  ok: boolean
  stdout: string
  stderr: string
}> {
  return new Promise((resolve) => {
    execFile(config.pythonBin, [script, ...args], {
      ...
      if (error) resolve({ ok: false, stdout, stderr: String(stderr || error.message) })
...
export function apply(ctx: import('@deepseek-ai/cordis').Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'Archive the current DSH conversation to the workspace session_archives/ directory: decode '
    ...
    async execute(args) {
      const argv: string[] = []
      if (args.sessionId) argv.push('--session-id', String(args.sessionId))
      if (args.statusOnly) argv.push('--status')
  ...
  ctx.tools.register(defineTool({
    name: 'search_archive',
    description:
      'Full-text search over archived DSH conversations (workspace session_archives/*.md, falling '
